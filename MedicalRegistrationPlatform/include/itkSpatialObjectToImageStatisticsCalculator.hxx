...
::ComputeStatistics()
{
  ...
  typename CovarianceAlgorithmType::MatrixType covarianceMatrix
    = covarianceAlgorithm->GetCovarianceMatrix();
...
::Update()
{
  if ( !m_Image || !m_SpatialObject )
    {
    itkExceptionMacro(
      "SpatialObjectToImageStatisticsCalculator: set image AND spatialObject.");
    }
  ...
  // iterator
  if ( !strcmp(m_SpatialObject->GetTypeName().c_str(),
    ...
    typename MaskSOType::Pointer maskSpatialObject
      = dynamic_cast< MaskSOType * >( m_SpatialObject.GetPointer() );
...
::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "Internal Spatial Object Time: "
